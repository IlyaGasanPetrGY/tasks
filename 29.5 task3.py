...
def bfs(tree, start, desired):
    knots_set = []
    knots_deq = deque()
    knots_deq.append(start)
    while len(knots_deq) > 0:
        currentnumber = knots_deq.pop()
        if currentnumber == desired:
            return True
        childrens = tree[currentnumber]
        knots_set.append(start)

        for i in childrens:
            if ((i in knots_set) == False):
                knots_deq.appendleft(i)
